...
vector<Point2f> BallDetector::detectBalls(const Mat& image) {
    ...
    double gaussianTime = 0.0;
    double thresholdTime = 0.0;
    double findContoursTime = 0.0;
    double contourProcessTime = 0.0;
    double sortTime = 0.0;
    ...
    if (image.empty()) {
        ...
        printDetectTimeStats(totalTime, gaussianTime, thresholdTime, findContoursTime, contourProcessTime, sortTime);
        return {};
    }
    ...
    // 1. 高斯模糊（计时）
    TimePoint gaussianStart = Clock::now();
    GaussianBlur(gray, gray, Size(5, 5), 0);
    gaussianTime = Duration(Clock::now() - gaussianStart).count();

    // 2. 二值化（计时）
    TimePoint thresholdStart = Clock::now();
    threshold(gray, bin, brightThreshold_, 255, THRESH_BINARY);
    thresholdTime = Duration(Clock::now() - thresholdStart).count();

    // 3. 提取轮廓（计时）
    vector<vector<Point>> contours;
    TimePoint findContoursStart = Clock::now();
    findContours(bin, contours, RETR_EXTERNAL, CHAIN_APPROX_SIMPLE);
    findContoursTime = Duration(Clock::now() - findContoursStart).count();

    // 4. 轮廓处理（筛选+最小外接圆，计时）
    vector<Point2f> centers;
    TimePoint contourProcessStart = Clock::now();
    for (const auto& contour : contours) {
        Point2f center;
        float radius;
        minEnclosingCircle(contour, center, radius);
    ...
    contourProcessTime = Duration(Clock::now() - contourProcessStart).count();

    // 5. 排序（计时：区分8点新算法和原始排序）
    TimePoint sortStart = Clock::now();
    if (centers.size() == 8) {
        try {
            centers = getOrdered8Points(centers); // 假设getOrdered8Points返回排序后的点
        }
        catch (const exception& e) {
            cout << "新排序算法失败，使用原始排序: " << e.what() << endl;
            // 原始排序（异常后执行，计入排序时间）
            sort(centers.begin(), centers.end(), [](const Point2f& a, const Point2f& b) {
                const float row_thresh = 30.0f;
                return (fabs(a.y - b.y) < row_thresh) ? (a.x < b.x) : (a.y < b.y);
                });
        }
    }
    else {
    ...
    sortTime = Duration(Clock::now() - sortStart).count();
...
// 辅助函数：输出检测步骤的时间统计
void BallDetector::printDetectTimeStats(double total, double gaussian, double threshold,
    double findContours, double contourProcess, double sort) {
    cout << "\n=== 球检测时间统计 ===" << endl;
    cout << "总检测时间：" << fixed << setprecision(6) << total << " 秒" << endl;

    // 计算占比（避免总时间为0导致除0）
    auto getPercent = [total](double part) {
        return (total < 1e-9) ? 0.0 : (part / total) * 100;
        };

    cout << "1. 高斯模糊：" << gaussian << " 秒，占比：" << fixed << setprecision(2) << getPercent(gaussian) << "%" << endl;
    cout << "2. 二值化：" << threshold << " 秒，占比：" << getPercent(threshold) << "%" << endl;
    cout << "3. 提取轮廓：" << findContours << " 秒，占比：" << getPercent(findContours) << "%" << endl;
    cout << "4. 轮廓筛选（含最小外接圆）：" << contourProcess << " 秒，占比：" << getPercent(contourProcess) << "%" << endl;
    cout << "5. 排序（含8点新算法尝试）：" << sort << " 秒，占比：" << getPercent(sort) << "%" << endl;
    cout << "======================\n" << endl;
}
...
vector<Point2f> BallDetector::stereoMatch(
    ...
    // 检查相机内参矩阵
    cout << "cameraMatrix1 尺寸: " << cameraMatrix1.rows << "×" << cameraMatrix1.cols << endl;
    cout << "cameraMatrix2 尺寸: " << cameraMatrix2.rows << "×" << cameraMatrix2.cols << endl;
    cout << "cameraMatrix1 是否为空: " << (cameraMatrix1.empty() ? "是" : "否") << endl;
    cout << "cameraMatrix2 是否为空: " << (cameraMatrix2.empty() ? "是" : "否") << endl;

    // 检查旋转矩阵
    cout << "R 尺寸: " << R.rows << "×" << R.cols << endl;
    cout << "R 是否为空: " << (R.empty() ? "是" : "否") << endl;

    // 检查平移向量（关键！）
    cout << "T 尺寸: " << T.rows << "×" << T.cols << endl;
    cout << "T 是否为空: " << (T.empty() ? "是" : "否") << endl;

    if (!T.empty()) {
        cout << "T 的数据类型: " << T.type() << " (应该是6，表示CV_64F)" << endl;
        cout << "T 的值: " << endl;
        for (int i = 0; i < T.rows; i++) {
            for (int j = 0; j < T.cols; j++) {
                cout << "  T(" << i << "," << j << ") = " << T.at<double>(i, j) << endl;
            }
        }
    }

    // 检查点数
    cout << "leftPoints 数量: " << leftPoints.size() << endl;
    cout << "rightPoints 数量: " << rightPoints.size() << endl;
    cout << "========================\n" << endl;
    ...
    */
    ...
    E = T_x * R;  // 本质矩阵 E = [T]_× * R

    // 从本质矩阵计算基础矩阵
    F = cameraMatrix2.inv().t() * E * cameraMatrix1.inv();
