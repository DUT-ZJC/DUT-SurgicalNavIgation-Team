...
/************************************************************************/
// 普通注释块，说明该头文件功能：基于厂商 C 接口再封一层 C++ 类，便于使用

#ifndef _MV_CAMERA_H_
#define _MV_CAMERA_H_
// 头卫士：防止头文件被重复包含导致的重定义错误。第一次包含时定义宏 _MV_CAMERA_H_；后续再包含会被 #ifndef 拦住

#include "MvCameraControl.h"
#include <string.h>
// 引入厂商 SDK 的 C 接口头（里边定义了各种 MV_CC_* 结构体、函数、宏，比如设备信息、图像结构等）
// <string.h > 提供 C 风格字符串函数（如 memcpy, strlen），说明本类里可能会用到 C 字符串

#ifndef MV_NULL
#define MV_NULL    0
#endif
// 定义 MV_NULL 宏，表示空指针,如果 SDK 或系统未定义，就这里兜底定义一个。

/*声明一个 C++ 类 CMvCamera，用于对底层 C 接口进行面向对象封装。

构造函数 CMvCamera()：创建对象时调用，通常做资源初始化（比如把句柄设为 MV_NULL）。

析构函数 ~CMvCamera()：对象销毁时调用，通常做资源释放（关闭设备、销毁句柄）。这体现 RAII 思想。*/
